...
def _available_markets(user_id: int) -> list[dict]:
    """Только рынки БЕЗ привязки к Surf Coffee (см. market.surf_spot_key,
    /add_project) — для подключённых рынков план тянется автоматически
    (см. revenue.daily_plan), вносить его вручную незачем и даже не
    предлагается, включая владельцу."""
    markets = list_markets() if is_owner(user_id) else get_markets_for_manager(user_id)
    return [m for m in markets if not m.get("surf_spot_key")]
...
async def on_set_monthly_plan_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    выручке/чекам на месяц вперёд для точки, НЕ подключённой к системе
    учёта Surf Coffee (см. /add_project). Для подключённых точек план
    бот забирает сам — эта команда для них просто не нужна. Тот же диалог
    запускается автоматически 25-го числа (см. send_monthly_plan_requests);
    ручной запуск доступен в любой день, если план нужно поправить."""
    ...
    if not markets:
        await update.effective_message.reply_text(
            "Нет рынков, где план вносится вручную — либо рынков вообще нет, либо все подключены к системе учёта "
            "Surf Coffee (там план бот забирает сам)."
        )
        return
...
async def send_monthly_plan_requests(bot: Bot) -> None:
    """25-го числа (см. main.py) бот сам просит у управляющего план по
    выручке/чекам на следующий месяц — только для рынков БЕЗ привязки к
    Surf Coffee, для остальных план и так подтянется сам."""
    for market in list_markets():
        if market.get("surf_spot_key"):
            continue

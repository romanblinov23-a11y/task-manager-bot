...
from bot.market_operator import on_set_operator_reply
...
def _find_project_by_assignee(name: str) -> str | None:
    ...
    (раздел 2.2: "по исполнителю, если он уже встречается в одном из проектов")."""
    if not name:
        return None
    found = {
        project
...
async def on_private_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if await on_set_operator_reply(update, context):
        return

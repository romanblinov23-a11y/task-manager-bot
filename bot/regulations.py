...
4. Согласование
Как только анкета заполнена (сотрудником или тобой), отчёт приходит тебе на согласование — с готовым текстом и двумя кнопками:
— «✅ Согласовать» — отправляю дальше Роме;
— «✏️ Редактировать» — можно точечно поправить любое поле (бот переспросит именно его, остальное не тронет).
Если анкету заполнял(а) ты сам(а) — этот шаг пропускается, отчёт сразу уходит Роме.

5. Согласование Романом
После тебя отчёт уходит Роме с кнопками «✅ Согласовать» / «💬 Запросить дополнения». Если Рома просит дополнения — я пришлю тебе его комментарий и тот же пикер полей для правки, что и в п.4; после правки отчёт снова уходит Роме.

6. Рассылка — 10:00 следующего дня
Как только Рома согласовал отчёт, на следующее утро в 10:00 я сам разошлю его в привязанные чаты (финпартнёры, команда точки). Здесь твоё участие заканчивается.
...
Раз в смену (по графику, который ведёт Управляющий) кто-то один сдаёт вечерний отчёт по точке — так финпартнёры и Рома видят, как прошла смена.
...
3. Анкета
По порядку отвечаю на вопросы: выручка (общая/наличные/безнал), средний чек, гости, три статьи списаний, среднее время отдачи заказов, шесть текстовых комментариев о смене и состав смены. Формат сумм — как в примере в самом вопросе (пробелы между тысячами не обязательны, дробная часть — через запятую или точку). Если где-то ошибся(лась) — я сам замечу нестыковку (например, если наличные + безнал не сходятся с общей выручкой) и предложу выбрать, какое поле поправить, — не нужно пересчитывать вручную.
...
5. Дальше
После анкеты отчёт уходит на согласование Управляющему, затем — Роме. Дальше ты не участвуешь: если что-то нужно поправить, Управляющий сделает это сам или уточнит у тебя лично.

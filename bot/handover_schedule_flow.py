...
from monitoring.handover_schedule import set_handover_schedule
from monitoring.managers import get_market_supervisor, get_markets_for_manager, is_handover_editor, is_owner
...
def _market_pick_keyboard(markets: list[dict]) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton(m["name"], callback_data=f"hosched_market:{m['id']}")] for m in markets])


def _confirm_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("✅ Подтвердить", callback_data="hosched_confirm"), InlineKeyboardButton("❌ Отмена", callback_data="hosched_cancel")]]
    )


def _instructions_text(market_name: str) -> str:
    return (
        f"Пришли график пересменок на 2 недели вперёд для «{market_name}» — кто сдаёт пересменку в какой день.\n\n"
...
async def on_set_handover_schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/set_handover_schedule — управляющий (или владелец) грузит график
    пересменок на 2 недели вперёд. Тот же диалог запускается автоматически
    14-го и в последний день месяца (см. send_handover_schedule_requests)."""
    user = update.effective_user
    if not is_handover_editor(user.id):
        await update.effective_message.reply_text(
            "Загружать график пересменок может только владелец или Управляющий с выданным блоком «Пересменка»."
        )
        return
    ...
    await update.effective_message.reply_text("По какому рынку загружаем график пересменок?", reply_markup=_market_pick_keyboard(markets))


async def on_set_handover_schedule_market_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def on_set_handover_schedule_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    lines.append("")
    lines.append(f"Сохранить график пересменок на «{market['name']}»?")
    await update.effective_message.reply_text("\n".join(lines), reply_markup=_confirm_keyboard())
    return True


async def on_set_handover_schedule_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    ...
    set_handover_schedule(state["market_id"], [(date_iso, user_id) for date_iso, user_id, _ in state["entries"]])
    await query.edit_message_text(f"✅ График пересменок сохранён: {len(state['entries'])} дат.")


async def on_set_handover_schedule_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
...
async def send_handover_schedule_requests(bot: Bot) -> None:
    """Раз в две недели (14-е и последний день месяца, см. main.py) бот сам
    просит у управляющего каждого рынка график пересменок на ближайшие
    2 недели — независимо от графика вечерних отчётов (см.
    send_shift_schedule_requests), это разные графики."""
    for market in list_markets():
        supervisor = get_market_supervisor(market["id"])
        if not supervisor or not is_handover_editor(supervisor["telegram_user_id"]):
            continue
